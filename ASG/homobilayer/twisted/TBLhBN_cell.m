%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Computes the moire cell for twisted bilayer hBN (TBLhBN) given two 
% integers n and m. Here, we assume that m=n+1.
%
% It generates six output files: 
...
%   5) potential.dat file to be used as input for TB_free.x
%	6) lammps geometry file
%
...
%       relative lines
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [nat,pos,pos2] = TBLhBN_cell(n, a, z, plot_f, kpath, nk, write_positions, write_potential, write_cart, write_lammps_input, write_kpath,phase)
   % Setting parameters
   m=n+1;
   ...
   d2 = zeros(nat/2,1);
   d2(1:nat/2) = 1.0+z; %dmin+delta*rlri2(1:nat/2);

   if (write_cart)
      if (plot_f=='xyz')
         write_xyz(nat,filename,alatL,id1,id2,pos,pos2)
      elseif (plot_f=='xsf')
         write_xsf(nat,filename,alatL,z,id1,id2,5,7,'B','N',pos,pos2)
      end
   end
   ...
   if (write_lammps_input)   
      ...
      m(3) = 10.811;
      m(4) = 14.0067;
      write_lammps(nat,filename,alatL,z,id1,id2,4,m,'B','N',pos,pos2)
   end
   %figure
