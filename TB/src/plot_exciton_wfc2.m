function [supercell,psi_M1,psi_M2,psi_k1,psi_k2] = plot_exciton_wfc2(index_orb_up,index_orb_down,r_h, orb_h_up, orb_h_down, Acvk, ...
...
s_h = size([orb_h_up.Ham_index],2);
Cv_u = zeros(dim,s_h);
Cv_d = zeros(dim,s_h);
Cv_sud = zeros(dim);


Cc_u = zeros(dim,5,Nat);
Cc_d = zeros(dim,5,Nat);
Cc_sud = zeros(dim);
for idim = 1 : dim
   psi_vk_rh_up = Cv([orb_h_up.Ham_index], indv(idim), indk(idim));
   psi_vk_rh_down = Cv([orb_h_down.Ham_index], indv(idim), indk(idim));
   Cv_u(idim,:) = psi_vk_rh_up'; 
   Cv_d(idim,:) = psi_vk_rh_down';
end

Cv_sud = Cv_u * Cv_u' + Cv_d * Cv_d';

for iat = 1 : Nat
   for idim = 1 : dim
       psi_ck_re_up = Cc([index_orb_up{iat}], indc(idim), indk(idim));
       psi_ck_re_down = Cc([index_orb_down{iat}], indc(idim), indk(idim));
       psi_ck_re_up(numel([1 2 3 4 5])) = 0;
       psi_ck_re_down(numel([1 2 3 4 5])) = 0;
       Cc_u(idim,:,iat) = psi_ck_re_up';
       Cc_d(idim,:,iat) = psi_ck_re_down';
   end
end

tic
for is = 1 : tot_r
      Cc_sud = Cc_u(:,:,nat_index(is)) * Cc_u(:,:,nat_index(is))' + Cc_d(:,:,nat_index(is)) * Cc_d(:,:,nat_index(is))';
      CvCc_s = conj(Cc_sud).*Cv_sud;
      re_rh = supercell(is,:) - r_h;
      for idim = 1 : dim
             eikdotR = exp(1i*(dot(all_kpts(indk(idim),:), re_rh)));
             sum_M1(idim) =  Acvk(idim,1) * eikdotR ;
             sum_M2(idim) =  Acvk(idim,2) * eikdotR ;
      end
      psi_M1(is) = psi_M1(is) + real(sum(sum(kron(sum_M1',sum_M1).*CvCc_s)));
      psi_M2(is) = psi_M2(is) + real(sum(sum(kron(sum_M2',sum_M2).*CvCc_s)));
end
